<x-layout-admin>
    ...
    <section class="bg-white dark:bg-gray-800">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
            ...
            <form action="{{ route('category.update', $category->id) }}" method="post" id="create">
                @csrf
                @method('put')
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="color" id="color" value="{{ old('color', $category->color) }}"
                            @if ($errors->has('color')) class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 appearance-none dark:text-white dark:border-red-500 border-red-600 dark:focus:border-red-500 focus:outline-none focus:ring-0 focus:border-red-600 peer"
                        @else
                       class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" @endif
                            placeholder="Type color article">
                        @error('color')
                            <span id="nameError" class="text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
